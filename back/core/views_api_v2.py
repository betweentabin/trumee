...
# Admin API Endpoints for seminars, publications, news
@api_view(['GET', 'POST'])
@permission_classes([AllowAny])  # 一時的に認証を無効化してテスト
def admin_seminars(request):
    ...
    if request.method == 'GET':
        ...
        mock_seminars = [
            {
                'id': 1,
                'title': 'Webデザイン基礎セミナー',
                'description': 'HTML/CSSからはじめるWebデザインの基礎を学びます',
                'date': '2025-09-15',
                'start_time': '10:00',
                'end_time': '16:00',
                'location': 'オンライン',
        ...
        return Response({
            'success': True,
            'data': mock_seminars,
            'total': len(mock_seminars),
            'page': page,
            'per_page': per_page
        })
    
    # POST処理（セミナー作成）
    return Response({'success': True, 'message': 'セミナーが作成されました'})
...
def admin_publications(request):
    ...
    if request.method == 'GET':
        ...
        mock_publications = [
            {
                'id': 1,
                'title': '経済統計レポート 2025年版',
                'description': '最新の経済統計をまとめたレポートです',
                'category': 'report',
                'file_path': '/uploads/reports/2025-economic-report.pdf',
                'file_size': 2048576,
            ...
            },
            {
                'id': 2,
                'title': '金融市場分析',
                'description': '金融市場の最新動向分析',
                'category': 'analysis',
                'file_path': '/uploads/analysis/financial-market-2025.pdf',
                'file_size': 1536000,
        ...
        return Response({
            ...
            'total': len(mock_publications),
            'page': page,
            'per_page': per_page
        })
    
    return Response({'success': True, 'message': '刊行物が作成されました'})
...
def admin_news(request):
        ...
        mock_news = [
            {
                ...
                'content': '弊社では新しいコンサルティングサービスを開始いたします。',
                'category': 'service',
                'status': 'published',
                'featured_image': '/images/news/service-launch.jpg',
                'published_date': '2025-09-01',
                'created_at': '2025-09-01T10:00:00Z'
            },
            {
                'id': 2,
                'title': 'セミナー開催のお知らせ',
                'content': '経済動向に関するセミナーを開催いたします。',
                'category': 'event',
                'status': 'published',
                'featured_image': '/images/news/seminar-announcement.jpg',
    ...
    return Response({'success': True, 'message': 'ニュースが作成されました'})
