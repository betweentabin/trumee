...
const useScoutSeeker = (
  ...
  const useMessage = (
    ...
    useMutation({
      mutationFn: (messageData) => sendMessage(receiverId, messageData),
      onSuccess: onSuccess ?? emptyHandler,
      onError: onError ?? emptyHandler,
    });
...
const updateProfile = (type: string) => async (data: any) => {
  ...
  // Add UID to data before sending
  const payload = {
    ...data,
    uid: uid,
  };
  ...
  const _res = await fetch(
    type === "seeker" ? "http://85.131.248.214:9000/api/seeker/profile/" : "/api/business/profile",
    {
      method: "POST",
...
const sendMessage = async (receiverId: number, data: any) => {
  ...
  const _res = await fetch("http://85.131.248.214:9000/api/message/", {
    method: "POST",
...
const sendMessageToUser = async (data: any) => {
  ...
  const company = data.detail.email;
  const _res = await fetch("http://85.131.248.214:9000/api/message/tocompany/", {
    method: "POST",
    body: JSON.stringify({
      message: message,
      company: company
