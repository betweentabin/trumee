...
const getSeekerProfile = async () => {
  ...
  const response = await axios.post(`http://85.131.248.214:9000/api/auth/history/`, {
      headers: {
...
const getScoutedSeekers = async () => {
  const token = localStorage.getItem('token')
  const _res = await fetch("http://85.131.248.214:9000/api/business/scout/", {
    method: "GET",
    headers: {
...
const getAppliedSeekers = async () => {
  const token = localStorage.getItem('token')
  const response = await fetch(`http://85.131.248.214:9000/api/business/search/applied_users/`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      "X-Requested-With": "XMLHttpRequest",
...
const getMessagesForMultiTypes =
  (typeList: string[], receiverId?: number) => async () => {
    console.log(typeList, receiverId,'message');
  ...
  const getMyResume = async () => {
    ...
    const response = await fetch("http://85.131.248.214:9000/api/get-resume-data/", {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
