...
export default function ResumeAdvicePage() {
  ...
  const load = useCallback(async () => {
    try {
      const url = `${buildApiUrl("/advice/messages/")}?subject=resume_advice`;
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl("/advice/messages/"), {
        ...
        body: JSON.stringify({ subject: "resume_advice", content: input.trim() }),
      });
      if (!res.ok) return;
      setInput("");
      await load();
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow border p-6">
        <h1 className="text-xl font-semibold mb-4">職務経歴書に関するアドバイス</h1>
