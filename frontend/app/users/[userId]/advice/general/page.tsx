...
export default function GeneralAdvicePage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const url = `${buildApiUrl("/advice/messages/")}?subject=advice${userId ? `&user_id=${encodeURIComponent(userId)}` : ""}`;
      ...
      setMessages(mapped);
      // 既読化（管理者側は対象ユーザーのスレッドを開いたタイミングで既読）
      try { await fetch(buildApiUrl("/advice/mark_read/"), { method: "POST", headers: getApiHeaders(token), body: JSON.stringify({ subject: "advice" }) }); } catch {}
    } catch { /* noop */ }
  }, [token, params?.userId]);
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch(buildApiUrl("/advice/messages/"), {
        ...
        body: JSON.stringify({ subject: "advice", content: input.trim(), ...(userId ? { user_id: userId } : {}) }),
      });
      if (!res.ok) return;
      setInput("");
      await load();
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow border p-6">
        <h1 className="text-xl font-semibold mb-4">アドバイス（汎用）</h1>
