...
import { FaMinus, FaPlus, FaLightbulb } from "react-icons/fa";
...
export default function FuturePlansPage() {
  ...
  const handleGenerate = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      setDraft(`将来やりたいこと\n\n【ビジョン】\n${vision}\n\n【実現のためのステップ】\n${steps || '必要なスキル習得とプロジェクト経験を積む計画です。'}\n\n上記を通して中長期的に価値提供を高めていきます。`);
      setLoading(false);
  ...
  const sendThread = async () => {
    ...
    try {
      const res = await fetch(buildApiUrl('/advice/messages/'), { method: 'POST', headers: getApiHeaders(token), body: JSON.stringify({ subject: 'advice', content: JSON.stringify({ type: 'future_plan', message: text }) }) });
      ...
      <div className="max-w-7xl mx-auto">
        <nav className="text-sm text-gray-500 mb-4" aria-label="breadcrumb">
          <ol className="flex items-center gap-2">
            ...
            <li>›</li>
            <li className="text-gray-800">将来やりたいこと</li>
          </ol>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <aside className="lg:col-span-3">
            <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
              {navItems.map((t) => (
                <button key={t} onClick={()=>router.push(to(toPath(t)))} className={`w-full text-left px-4 py-3 border-b last:border-b-0 ${t==='将来やりたいこと'?'bg-[#FFF7E6] font-semibold':'hover:bg-gray-50'}`}>{t}</button>
              ))}
            </div>
          </aside>
          ...
          <main className="lg:col-span-9 space-y-6">
            <div className="mb-2">
              <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
                <FaLightbulb className="text-[#FF733E]" /> 将来やりたいこと
              </h1>
              <p className="text-gray-600 mt-2">中長期のキャリアビジョンを言語化し、応募先との接点を作ります。</p>
            </div>

            <div className="bg-white rounded-lg shadow-sm border">
              <button onClick={()=>setOpenGuide(!openGuide)} className="w-full flex items-center justify-between px-4 py-3 font-semibold">ガイド {openGuide ? <FaMinus/> : <FaPlus/>}</button>
              {openGuide && (
                <div className="px-4 pb-4 text-sm text-gray-700 space-y-2">
                  <p>構成例: ビジョン → 根拠となる経験 → 必要スキルと習得計画 → 応募先で実現したいこと。</p>
                </div>
              )}
            </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea value={vision} onChange={(e)=>setVision(e.target.value)} className="w-full h-28 p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">実現までのステップ（任意）</label>
                    <textarea value={steps} onChange={(e)=>setSteps(e.target.value)} className="w-full h-24 p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
                  </div>
